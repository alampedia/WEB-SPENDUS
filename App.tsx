...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<AppSettings>({
    ...
    adminTitle: 'ADMIN SMP',
    adminSubtitle: 'Panel Sekolah Menengah Pertama',
    footerText: 'Digital Assessment System (DAS) @2026 | akhmadnasor',
    themeColor: '#2459a9',
    gradientEndColor: '#60a5fa',
    logoStyle: 'circle',
    schoolLogoUrl: 'http://lh3.googleusercontent.com/d/1bh9rNBQ0IK9HvNGTwpsGQRcnEaibm_3Y',
    antiCheat: { isActive: true, freezeDurationSeconds: 15, alertText: 'Violation!', enableSound: true, antiFastSubmit: false, minWorkTimeMinutes: 10 },
    globalToken: { isActive: false, token: 'TOKEN2024' }
  });
  ...
  const loadSettings = async () => {
    try {
      ...
      link.href = s.schoolLogoUrl || 'http://lh3.googleusercontent.com/d/1bh9rNBQ0IK9HvNGTwpsGQRcnEaibm_3Y';
    } catch (error) {
        ...
        <div className="min-h-screen relative font-sans overflow-hidden" style={loginBgStyle}>
              ...
              <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                      ...
                      <div>
                          ...
                          <p className="text-xs text-blue-100 opacity-90">{settings.adminSubtitle || 'Digital Assessment System (DAS) - Jenjang SMP'}</p>
                      </div>
                  </div>
              </div>
          </header>
          ...
          <div className="fixed bottom-6 w-full text-center z-20 pointer-events-none">
              <span className="inline-block bg-white/80 backdrop-blur rounded-full px-4 py-1.5 text-xs font-semibold shadow-lg border border-white/50" style={{ color: settings.themeColor }}>
                {settings.footerText || 'Digital Assessment System (DAS) @2026 | akhmadnasor'}
              </span>
          </div>

        </div>
      ) : (currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.PROCTOR) ? (
        <AdminDashboard 
          user={currentUser} 
